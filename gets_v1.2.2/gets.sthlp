...
comprised of all independent variables the user considers potentially important, plus nonlinearities and lags), {cmd:gets} 
...
{p 6 6 2}
{hi:gets} allows the user to run the model prediction algorithm for time-series, cross-sectional, or panel data models.  The {hi:gets} command runs a
...
operators. If factor variable operators are used with the {cmd:xt} option, {cmd:noserial} should be specified.  The {hi:gets} command accepts 
...
{phang2}{cmd:. gets y5 `xvars' `lags' l.y5 l2.y5 l3.y5 l4.y5, ts}{p_end}
